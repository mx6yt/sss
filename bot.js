...
client.on('message', message => {
    ...
    if(message.content.startsWith(prefix + "ping")) { // حقوق مداكس تو
        if (message.author.bot) return;
        if(!message.channel.guild) return;
var ping = `${Date.now() - message.createdTimestamp}`
                const E1ping = new Discord.RichEmbed()
        .setTitle('ــــــــــــــــــــــــــــــ')
        .addField(`**BOT Ping Is** :__${ping}📶__`,"ــــــــــــــــــــــــــــــ")
...
rebel.channel.send(" , <@"+rebel.author.id+"> I Try To Remove Afk Mode For You.");     } else {
    ...
    if (rebel.content.startsWith(prefix + 'afk ')||rebel.content === 'afk') {
...
// THIS  MUST  BE  THIS  WAY
 ...
 ╱╭╮╭╮╱╱╱╱╭╮╭━╮╱╱╱╱╱╱╭━━╮╱╱╱
♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ ♕ 
 __created By__: <@493559451731623961>

Server Support : هنا رابط سيرفر الدعم الفني

bot link : هنا رابط بوتك
`);
...
client.on('guildMemberAdd', member => {
  ...
  member.guild.fetchInvites().then(guildInvites => {
    ...
    if(!logChannel) return;
      setTimeout(() => {
    logChannel.send(`
**Welcom To Server ${member} 🌹✨
By Friend : <@${inviter.id}>**
`);{}
  },2000)
  fs.writeFile("./welcome.json", JSON.stringify(welcome), (err) => {
    if (err) console.error(err)
...
client.login('NjE0ODQxNjczMTA0NzUyNjQy.Xd68nA.bBf3bV9VBEl6hEneeIMnio31qVg');
